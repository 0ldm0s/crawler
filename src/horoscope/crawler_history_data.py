...
parent_dir = 'horoscope-crawler/history/'
...
url = 'https://www.horoscope.com/us/horoscopes/%s/horoscope-archive.aspx?sign=%s&laDate=%s'
upload_data = {}
yyyy_mm_dd = '%Y-%m-%d'
yyyymmdd = '%Y%m%d'


def get_today(format_str):
    return date.today().strftime(format_str)


def crawler_history_date():
    """
    抓取历史星座信息
    :return: 
    """
...
def storage_json(json_str):
    ...
    os.chdir(p_dir)
    with open(get_today(yyyy_mm_dd) + '.txt', 'w') as f:
